"""
This file is adapted from diffusers' Z-Image transformer implementation.
"""
...
class ZImageTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attention = Attention(
            ...
            bias=False,
            out_bias=False,
            processor=IPAZImageAttnProcessor(hidden_size=dim),
        )
    ...
    def forward(
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer in self.noise_refiner:
                x = self._gradient_checkpointing_func(layer, x, x_attn_mask, x_freqs_cis, adaln_input, image_emb)
        else:
            for layer in self.noise_refiner:
                x = layer(x, x_attn_mask, x_freqs_cis, adaln_input, image_emb=image_emb)
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer_idx, layer in enumerate(self.layers):
                unified = self._gradient_checkpointing_func(
                    layer, unified, unified_attn_mask, unified_freqs_cis, adaln_input, image_emb
                )
            ...
            for layer_idx, layer in enumerate(self.layers):
                unified = layer(unified, unified_attn_mask, unified_freqs_cis, adaln_input, image_emb=image_emb)
                if controlnet_block_samples is not None:
                    if layer_idx in controlnet_block_samples:
                        unified = unified + controlnet_block_samples[layer_idx]
